...
struct runqueue {
	...
	task_t *curr, *idle;
	prio_array_t *active, *expired, *shorts, *overdue, arrays[4]; // OUR CODE
...
// OUR COMMENT - enqueue changes the array of the task to the given array. we can use it to change to SHORT
...
// OUR COMMENT - activate alaways sets the task's array to be active
...
static int setscheduler(pid_t pid, int policy, struct sched_param *param) {
	...
	} else { 			// the new policy is SHORT
		p->policy = policy;
		p->prio = lp.sched_short_prio;
		p->requested_time = requested_time;

		prio_array_t *array_s = rq->shorts;
		enqueue_task(p, array_s);
		rq->nr_running++;
	}
...
void __init sched_init(void) {
	...
	for (i = 0; i < NR_CPUS; i++) {
		...
		for (j = 0; j < 4; j++) { // OUR CODE (4 instead of 2)
			array = rq->arrays + j;
